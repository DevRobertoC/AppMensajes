...
def confirm():
    try:
        ...
        if request.method == 'POST': # ? 
            ...
            if attempt is not None:
                db.execute(
                    'UPDATE forgotlink SET state=? WHERE id=?', (utils.F_INACTIVE, attempt['id']) #Actualziar el registro en fprgolink y cambiar el estado por inactivo
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'UPDATE user SET password=?, salt=? WHERE id=?', (hashP, salt, attempt['userid']) # Actualizar el regostro del usuario y cambiar la contraseña y el salt
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'GET':  # ? 
            ...
            db = get_db()  # ?
            attempt = db.execute(
                'SELECT * FROM forgotlink WHERE challenge=? AND state=? AND CURRENT_TIMESTAMP BETWEEN created AND validuntil', (number, utils.F_ACTIVE) # Igual al anterior selec
            ).fetchone()
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            db = get_db()
            user = db.execute(
                'SELECT * FROM user WHERE email=?', (email,) # consulta que devuelva si el usuario existe
            ).fetchone()
            ...
            if user is not None:
                ...
                db.execute(
                    'UPDATE forgotlink SET state=? WHERE userid=?', # actualizar forgolink 
                    (utils.F_INACTIVE, user['id'])
                )
                db.execute(
                    'INSERT INTO forgotlink(userid, challenge, state) VALUES(?,?,?)', # insertar registro que permite al usuario cambiar su contraseña
